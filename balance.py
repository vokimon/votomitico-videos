...
def create_balance(self):
    # Use the smallest dimension for scaling
    s = min(config.frame_width, config.frame_height)
    self.base_scale = s
    self.arm_angle = 0  # initial arm angle
    ...
    ).shift(DOWN * (self.support_height + base_triangle_height))
    ...
    # Plate shape (trapezoid) generator
    ...
    # Chains connecting arm tips to plates
    ...
    # Group all visible elements for easy control and transitions
...
class DebugBalanceScene(Scene):
    def construct(self):
        # Build balance and elements
        create_balance(self)

        # Run each animation phase manually (simulating audio sync)
        show_balance(self)
        self.wait(0.5)
